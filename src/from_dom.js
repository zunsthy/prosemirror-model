...
class NodeContext {
  constructor(type, attrs, marks, pendingMarks, markStack, solid, match, options) {
    this.type = type
    this.attrs = attrs
    this.solid = solid
  ...
  applyPending(nextType, useStack) {
    ...
    if (useStack) {
      this.markStack.top = [this.activeMarks, this.pendingMarks];
    }
  }
  ...
  addDOM(dom) {
    if (dom.nodeType == 3) {
      ...
      if (this.useStack) {
        ...
        this.top.activeMarks = this.top.markStack.top[0]
      }
    }
  }
